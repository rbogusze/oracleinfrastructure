...
--   PO Approval item type with Age = 0.
  ...
  CURSOR po1 IS
  SELECT /*+ FULL(i) parallel(i,8) FULL(ias) parallel(ias,8) */ DISTINCT
         ias.item_type,
         ias.item_key
  FROM wf_items i,
  ...
  AND   parent_item_key IN (
          SELECT /*+ FULL(i1) parallel(i1,8) FULL(h) parallel(h,8) */ i1.item_key
          FROM wf_items i1,
               po_headers_all h
          ...
          UNION
          SELECT /*+ FULL(i1) parallel(i1,8) FULL(r) parallel(r,8) */ i1.item_key
          FROM wf_items i1,
               po_releases_all r
  ...
  AND   ias.activity_status <> 'COMPLETE'
  AND   NOT EXISTS (
          SELECT /*+ FULL(h) parallel(h,8) */ 1 FROM po_headers_all h
          WHERE h.wf_item_key = i.parent_item_key
          UNION
          SELECT /*+ FULL(h) parallel(h,8) */ 1 FROM po_releases_all r
  ...
  AND   NOT EXISTS (
          ...
          UNION
          SELECT /*+ FULL(r) parallel(r,8) */ 1 from po_releases_all r
  ...
  CURSOR po_ag IS
  SELECT  /*+ FULL(i) parallel(i,8) */ i.item_type,
         i.item_key
  FROM wf_items i
  ...
  CURSOR req3 IS
  SELECT /*+ FULL(i) parallel(i,8) FULL(ias) parallel(ias,8) FULL(r) parallel(r,8) */ DISTINCT
         ias.item_type,
         ias.item_key
  FROM wf_item_activity_statuses ias,
       wf_items i,
