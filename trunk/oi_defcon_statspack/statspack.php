<?php
/* $Id: statspack.php,v 1.3 2011/07/27 20:21:10 remikcvs Exp $ */

// Draw Pictures with section data
function draw_chart_section($dir, $chart_data, $chart_leg, $data_module, $section_name, $data_values_executions, $timestamp, $width, $height) {
  ...
  for($j=0;$j<count($chart_data);$j++)
  {
    ...
    print "</td><td>";
    print "<a href=\"hash_history.php?dir=" . $dir . "&hash_value=" . $each_chart_leg["value"] . "\" >" . $each_chart_leg["value"] . "</a>"; 
    print "</td><td>";
...
$timestamp = substr ( $filename, 5, 10);
...
while (!feof($fh))
{
  ...
  // Spot sections start
  if (strstr ( $data, "SQL ordered by CPU ")) { echo "<font color='red'>"; $section = 1; }
  ...
  if ( $section ) {
    ...
    list($trash1, $trash2, $trash3, $trash4, $trash5, $trash6, $hash_value) = split(' ',$data);
    ...
    if ( is_numeric($trash1) && is_numeric($trash2) && is_numeric($trash3) ) { 
      //echo "<BR> hash_value: $hash_value , section_name: $section_name , trash1: $trash1 <BR>";
      ...
      if ($section_name == "Buffer_Gets") {
        $data_values1[$data_values1_counter] = $trash1;
        $data_values1_label[$data_values1_counter] = $hash_value;
        $data_values1_module[$data_values1_counter] = $module_name;
        $data_values1_sql[$data_values1_counter] = $sql_text;
        $data_values1_executions[$data_values1_counter] = $trash2;
        $data_values1_elapsedtime[$data_values1_counter] = $trash6;
        $data_values1_counter++;
      }
      if ($section_name == "Physical_Reads") {
        $data_values2[$data_values2_counter] = $trash1;
        $data_values2_label[$data_values2_counter] = $hash_value;
        $data_values2_module[$data_values2_counter] = $module_name;
        $data_values2_sql[$data_values2_counter] = $sql_text;
        $data_values2_executions[$data_values2_counter] = $trash2;
        $data_values2_elapsedtime[$data_values2_counter] = $trash6;
        $data_values2_counter++;
      }
      if ($section_name == "CPU_Usage") {
        $data_values3[$data_values3_counter] = $trash1;
        $data_values3_label[$data_values3_counter] = $hash_value;
        $data_values3_module[$data_values3_counter] = $module_name;
      ...
      if ($section_name == "Elapsed_Time") {
        $data_values5[$data_values5_counter] = $trash1;
        $data_values5_label[$data_values5_counter] = $hash_value;
...
//show_array($data_values1_label);
//show_array($data_values1_elapsedtime);

echo "<table><tr><td>";

draw_chart_section($dir, $data_values3, $data_values3_label, $data_values3_module, "CPU_Usage", $data_values3_executions, $timestamp, 400, 500);
echo "</td><td>";
draw_chart_section($dir, $data_values1, $data_values1_label, $data_values1_module, "Buffer_Gets", $data_values1_executions, $timestamp, 400, 500);
echo "</td>";
echo "</tr><tr>";
echo "<td>";
draw_chart_section($dir, $data_values2, $data_values2_label, $data_values2_module, "Physical_Reads", $data_values2_executions, $timestamp, 400, 500);
echo "</td><td>";
draw_chart_section($dir, $data_values5, $data_values5_label, $data_values5_module, "Elapsed_Time", $data_values5_executions, $timestamp, 400, 500);
echo "</td></tr><tr><td>";
